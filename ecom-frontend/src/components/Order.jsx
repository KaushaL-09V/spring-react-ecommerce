            ...
            <table className="table table-hover mb-0">
              ...
              <tbody>
                {orders.length === 0 ? (
                  ...
                  orders.map((order) => {
                    // Some backends may name the item list differently. Be defensive.
                    const items = order.items || order.orderItems || order.itemResponses || order.responses || [];
                    ...
                    return (
                      <React.Fragment key={order.orderId}>
                        <tr>
                          ...
                          <td>{items.length}</td>
                          <td className="fw-bold">{formatCurrency(calculateOrderTotal(items))}</td>
                          <td>
                            <button 
                                  ...
                                  <table className="table table-sm table-bordered mb-0">
                                    ...
                                    <tbody>
                                      {items.map((item, index) => (
                                        <tr key={index}>
                                          <td>{item.productName || item.product?.name || item.name || 'Product'}</td>
                                          <td className="text-center">{item.quantity ?? '-'}</td>
                                          <td className="text-end">{formatCurrency(Number(item.totalPrice) || Number(item.price) || 0)}</td>
                                        </tr>
                                      ))}
                                      <tr className="table-info">
                                        <td colSpan="2" className="text-end fw-bold">Total</td>
                                        <td className="text-end fw-bold">{formatCurrency(calculateOrderTotal(items))}</td>
                                      </tr>
                                    </tbody>
                                  </table>
                                </div>
